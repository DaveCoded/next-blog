...
```js
import _ from "lodash"; // ❌ All of lodash ends up in the bundle
import { difference } from "lodash"; // ❌ Also includes all of lodash
import chunk from "lodash/chunk"; // ✅ Only the chunk method (and its dependences) in the bundle
```
...
> It is at this point that our hero comes to a fork in the highway of his destiny.
> No longer able to plough a straight furrow, he looks down the western path.
> The pass through its low crags looks easy to navigate, but there are many caves,
> the likes of which might hide Webpack goblins, and he thinks he espies the faded,
> dun bones of their victims.
> The eastern path quickly shoots up into a heady mountainscape. Its terrain looks arduous,
> its trees abstract, and syntactic. But the clouds. Icy peaks glint seductively, high above
> those clouds. He has never been above the clouds before. He has never risen so high. Now is his chance!
...
Find and replace won't do. No matter your regex skills. We need to be able to make changes based on the programming language's syntax. To specify, for example:

- If there are named imports from `foo-library`, and those imports are functions used in the file, remove them
