...
<p>
  <Dropcap>Often</Dropcap> in code, you handle arrays of
  items. Sometimes you know their types but not their
  number. Sometimes you even know the complete list of
  possible values, but not their number. What if that is the
  case and you want to sort them in a particular order? Not
  alphabetically, or by date, or from longest to shortest,
  or anything a computer can calculate. You want to sort
  them by something that seems, at least to a dumb computer,
  entirely arbitrary (laughable, even). Alphabetically by
  which letter maps to your favourite numbers between 1 and
  26. Or according to a list of label names for the form
  inputs in a UI? Yeah, that one. That's what happened to
  me.
</p>
